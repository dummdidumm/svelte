...
export function signal<T>(v: T, firewall: Computed<unknown> | null = null): Signal<T> {
	if (firewall !== null) {
...
export function read<T>(el: Signal<T> | Computed<T>, c: Computed<unknown> | null = context): T {
	if (c) {
		link(el, c);
...
export function runWithOwner<T>(owner: Computed<unknown> | null, fn: () => T): T {
	const oldContext = context;
	context = owner;
	try {
		return fn();
